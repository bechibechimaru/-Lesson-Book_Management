...
impl UserRepository for UserRepositoryImpl{
    ...
    async fn find_current_user(
        ...
        match row {
            Some(r) => {
                info!("Successfully found user with UserId: {:?}", current_user_id);
                Ok(Some(User::try_from(r)?))
            }
            None => {
                info!("No user found with UserId: {:?}", current_user_id);
                Ok(None)
            }
        }
    }
    ...
    async fn update_password(
        ...
        let mut tx = self.db.begin().await?; // トランザクションを作成
        // 'c　ライフタイム変数
    ...
    async fn update_role(&self, event: UpdateUserRole) -> AppResult<()> {
        ...
        if res.rows_affected() < 1{ // rows_affectedの記述で変更されたレコード数を取得できる
...
fn verify_password(password: &str, hash: &str) -> AppResult<()> {
    // password: 入力されたパスワード　
    // hash: 保存されているパスワード
    let valid: bool = bcrypt::verify(password, hash)?;
    if !valid{
